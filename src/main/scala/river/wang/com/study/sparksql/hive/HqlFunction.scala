...
/**
  ...
  */
object HqlFunction extends MyUtils {
  def main(args: Array[String]): Unit = {
    try {
      ...
      val df = sc.createDataFrame(datas, schema)

      import sc.implicits._

      val resultDf = df.groupBy("date").agg(countDistinct('id))
      println(resultDf.queryExecution)
      resultDf.show()

      val resultDf1 = df.groupBy("date").agg(sum('amount))
      println(resultDf1.queryExecution)
      resultDf1.show()
