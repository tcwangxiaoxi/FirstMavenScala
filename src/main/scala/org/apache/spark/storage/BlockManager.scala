...
private[spark] class BlockManager(
  ...
  private def reportBlockStatus(
    ...
    logInfo(s"================ tryToReportBlockStatus ending =============")
    if (needReregister) {
  ...
  private def tryToReportBlockStatus(
    ...
    if (info.tellMaster) {
      ...
      logInfo(s"================ tryToReportBlockStatus -> updateBlockInfo starting =============")
  ...
  : Seq[(BlockId, BlockStatus)] = {
    ...
    putBlockInfo.synchronized {
      ...
      try {
        ...
        logInfo("=================================== Actually put the values starting =============")
        ...
        size = result.size
        logInfo(s"===============size(${size}) Actually put the values ending =============")
        result.data match {
          case Left(newIterator) if putLevel.useMemory => valuesAfterPut = newIterator
          case Right(newBytes) => bytesAfterPut = newBytes
          case _ =>
        }
        logInfo("================ Keep track of which blocks are dropped from memory starting =============")
        ...
        logInfo("================ Keep track of which blocks are dropped from memory ending =============")
        val putBlockStatus = getCurrentBlockStatus(blockId, putBlockInfo)
        logInfo(s"================ getCurrentBlockStatus ending putBlockStatus(${putBlockStatus})=============")
        if (putBlockStatus.storageLevel != StorageLevel.NONE) {
          // Now that the block is in either the memory, externalBlockStore, or disk store,
          // let other threads read it, and tell the master about it.
          logInfo(s"================ putBlockStatus.storageLevel != StorageLevel.NONE starting =============")
          marked = true
          putBlockInfo.markReady(size)
          if (tellMaster) {
            logInfo(s"================ tellMaster starting =============")
            reportBlockStatus(blockId, putBlockInfo, putBlockStatus)
            logInfo(s"================ tellMaster ending =============")
          }
          updatedBlocks += ((blockId, putBlockStatus))
        }
        logInfo("================ tellMaster finish =============")
      } finally {
    ...
    logInfo("================ putBlockInfo.synchronized ending =============")
    ...
    logInfo("================ putLevel.replication > 1 ending =============")

    BlockManager.dispose(bytesAfterPut)

    logInfo("================ BlockManager.dispose ending =============")
