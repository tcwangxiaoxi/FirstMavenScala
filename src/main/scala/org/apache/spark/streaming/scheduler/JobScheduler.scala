...
class JobScheduler(val ssc: StreamingContext) extends Logging {
  ...
  private def handleJobStart(job: Job, startTime: Long) {
    logInfo("============================handleJobStart starting!!===========================")
    ...
    def run() {
      try {
        ...
        if (_eventLoop != null) {
          ...
          logInfo("============================_eventLoop.post(JobStarted) starting!!===========================")
          _eventLoop.post(JobStarted(job, clock.getTimeMillis()))
          logInfo("============================_eventLoop.post(JobStarted) ending!!===========================")

          // Disable checks for existing output directories in jobs launched by the streaming
          // scheduler, since we may need to write output to an existing directory during checkpoint
          // recovery; see SPARK-4835 for more details.
          PairRDDFunctions.disableOutputSpecValidation.withValue(true) {
            logInfo("============================job.run() starting!!===========================")
            job.run()
            logInfo("============================job.run() ending!!===========================")
          }
          _eventLoop = eventLoop
          if (_eventLoop != null) {
            logInfo("============================_eventLoop.post(JobCompleted) starting!!===========================")
            _eventLoop.post(JobCompleted(job, clock.getTimeMillis()))
            logInfo("============================_eventLoop.post(JobCompleted) ending!!===========================")
          }
          logInfo("============================_eventLoop != null ending!!===========================")
        } else {
          // JobScheduler has been stopped.
          logInfo("============================JobScheduler has been stopped.===========================")
        }
      } finally {
        logInfo("============================finally starting!!===========================")
